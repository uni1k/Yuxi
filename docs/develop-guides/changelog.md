...
- 扩展知识库与对话附件文档解析格式：新增 `.doc`、`.wps`、`.ofd` 上传识别支持，并补齐对话附件 `.xls/.xlsx` 解析入口；旧版 Office/WPS 文件先经 LibreOffice/soffice 归一为 DOCX/PDF，OFD 文件默认经内置 `ofdrw` 命令 `yuxi-ofdrw-ofd2images` 导出逐页图片，仍支持通过 `YUXI_OFD_TO_IMAGE_COMMAND` 覆盖为外部导出命令，再逐页复用既有 OCR 链路，Excel 文件继续复用既有 Docling 表格解析。
...
- 修复文档解析边界问题：后缀为 `.docx` 但实际 content type 为 `application/vnd.ms-word.document.macroEnabled.main+xml` 的启用宏 Word 文档会被识别并按 `.docm` 走 LibreOffice 转换；`.et` 等 WPS 表格转换需要 Java 运行时，生产镜像补充 `default-jre`、`libreoffice-java-common`、`libreoffice-calc-nogui` 与 `ca-certificates-java`；OFD 导出的逐页图片与扫描版 PDF 经 MinerU 返回空文本时自动回退到 `rapid_ocr`。
- 修复配置模块循环导入：`yuxi.config.app` 改为相对导入 `cache` 子模块，避免启动/热重载时因 `yuxi.config` 部分初始化而报 `cannot import name 'config'`。
